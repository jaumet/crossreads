...
package nlp.nicta.ner.classifier;
...
import nlp.nicta.ner.classifier.feature.FeatureMap;
import nlp.nicta.ner.data.NameType;
import nlp.nicta.ner.data.Phrase;
import nlp.nicta.ner.resource.Configuration;
...
public class NameClassifier {
	 ...
	 * and gives all phrases in the set a name type.
	 * 
	 * @param resultSet
	 * @return
	 */
	public NERResultSet process(NERResultSet resultSet) {
		// store the relationship of phrases in memory for further use
		HashMap<Phrase, HashSet<Phrase>> phraseInMemory =
			new HashMap<Phrase, HashSet<Phrase>>();
		
		// scoring
		ArrayList<ArrayList<Phrase>> phrases = resultSet.phrases;
		for(int si = 0; si < phrases.size(); si++) {
			// for each sentence
			ArrayList<Phrase> phrasesInSentence = phrases.get(si);
		...
		// copy the score of phrases that have relationships:
